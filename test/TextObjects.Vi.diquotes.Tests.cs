...
namespace Test
{
    ...
    {
        [SkippableTheory]
        [InlineData("a 'quoted' text", "di'", "a '' text", 3)]
        [InlineData("a \"quoted\" text", "di\"", "a \"\" text", 3)]
        [InlineData("\"quoted\" text", "di\"", "\"\" text", 1)]
        public void ViTextObject_diquotes(string input, string motion, string expected, int expectedCursorPos)
        {
            ...
            Test(expected, Keys(
                ...
                CheckThat(() => AssertCursorLeftIs(expectedCursorPos))
            ));
